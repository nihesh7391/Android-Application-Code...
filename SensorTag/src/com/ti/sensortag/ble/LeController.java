...
// ARV import static com.ti.sensortag.ble.Sensor.SIMPLE_KEYS;
...
public enum LeController implements PropertyChangeListener {
  ...
  void onServicesDiscovered(final BluetoothDevice device, int status) {
    ...
    /* ARV if (isEnabledByPrefs(Sensor.BAROMETER)) {
      calibrateBarometer();
    }*/
  }
  ...
  public void calibrateBarometer() {
    writeQueue.queueRunnable(new Runnable() {
      public void run() {
// ARV        UUID configUuid = Sensor.BAROMETER.getConfig();
// ARV        BluetoothGattCharacteristic config = getService(Sensor.BAROMETER).getCharacteristic(configUuid);
 
        byte[] callibrationCode = new byte[] { 2 };
 
// ARV        boolean successLocalySetValue = config.setValue(callibrationCode);
// ARV        logEvent(successLocalySetValue, "Unable to locally set the enable code.");
 
// ARV        boolean success = bluetoothGatt.writeCharacteristic(config);
// ARV        logEvent(success, "Unable to initiate the write that configures " + Sensor.BAROMETER);
      }
    });
    writeQueue.queueRunnable(new Runnable() {
      public void run() {
// ARV        BluetoothGattCharacteristic calibrationCharacteristic = getService(Sensor.BAROMETER).getCharacteristic(SensorTag.UUID_BAR_CALI);
// ARV        boolean success = bluetoothGatt.readCharacteristic(calibrationCharacteristic);
// ARV        logEvent(success, "Unable to read calibration values.");
      }
    });
    writeQueue.queueRunnable(new Runnable() {
      public void run() {
...
// ARV        boolean successLocalySetValue = config.setValue(enableCode);
// ARV        logEvent(successLocalySetValue, "Unable to locally set the enable code.");
    ...
    leScanCallback = new LeScanCallback() {
      public void onLeScan(final BluetoothDevice device, int rssi, byte[] scanRecord) {
        // Only react to SensorTags
          Log.e(TAG, "xxxxxxxxxxx"+scanRecord);
          
        boolean isSensorTag = device.getName().equals("SensorTag"); //SensorTag
        if (!isSensorTag)
          return;
...
/* ARV    if (sensor == SIMPLE_KEYS) // Simple keys are always enabled.
      return;
*/
    writeQueue.queueRunnable(new Runnable() {
      @Override
