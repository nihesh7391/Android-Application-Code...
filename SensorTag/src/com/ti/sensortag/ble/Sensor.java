  ...
  IR_TEMPERATURE(UUID_IRT_SERV, UUID_IRT_DATA, UUID_IRT_CONF) {
    ...
    public void onCharacteristicChanged(BluetoothGattCharacteristic c) {
       ...
       */
         ...
         double touch = extractTouchFlag(c);
         model.setAmbientTemperature(touch);
    ...
    private double extractAmbientTemperature(BluetoothGattCharacteristic c) {
      int offset = 2;
      return shortUnsignedAtOffset(c, offset);     //ARV / 128.0;
    }

    private double extractTouchFlag(BluetoothGattCharacteristic c) {
        int offset = 4;
        return shortUnsignedAtOffset(c, offset);     //ARV / 128.0;
    }
    ...
    private double extractTargetTemperature(BluetoothGattCharacteristic c, double ambient) {
      ...
      double amb_act = ambient/100;
     
      /*ARV Vobj2 *= 0.00000015625;
...
/*
  ACCELEROMETER(UUID_ACC_SERV, UUID_ACC_DATA, UUID_ACC_CONF) {
    ...
    public void onCharacteristicChanged(final BluetoothGattCharacteristic c) {
      ...
      //Integer x0 = c.getIntValue(FORMAT_SINT8, 0);
      Integer x1 = shortSignedAtOffset1(c, 0); 
      //Integer y0 = c.getIntValue(FORMAT_SINT8, 1);
      Integer y1 = shortSignedAtOffset1(c, 2);
      //Integer z0 = c.getIntValue(FORMAT_SINT8, 2);
      Integer z1 = shortSignedAtOffset1(c, 4);

      double scaledX = (x1-1024)/341.333333333;
      double scaledY = (y1-1024)/341.333333333;
      double scaledZ = (z1-1024)/341.333333333;
      
      String acce = "X: "+ String.valueOf(scaledX)+" Y: "+ String.valueOf(scaledY)+" Z: "+ String.valueOf(scaledZ); 
      accwriteintoafile(acce);
...
/*HUMIDITY(UUID_HUM_SERV, UUID_HUM_DATA, UUID_HUM_CONF) {
  ...
  private static Integer shortSignedAtOffset(BluetoothGattCharacteristic c, int offset) {
    ...
    //ARV Integer upperByte = c.getIntValue(FORMAT_SINT8, offset + 1); // Note:
    Integer upperByte = c.getIntValue(FORMAT_UINT8, offset + 1);    // interpret
                                                                 // MSB as
                                                                 // signed.

   //ARV return (upperByte << 8) + lowerByte;
    return (lowerByte*10) + upperByte;
  }
  ...
  // Added by Nihesh for testing.
  private static Integer shortSignedAtOffset1(BluetoothGattCharacteristic c, int offset) {
	    Integer lowerByte = c.getIntValue(FORMAT_UINT8, offset);
	                                                                    // Note:
	    Integer upperByte = c.getIntValue(FORMAT_UINT8, offset + 1);    // interpret
	                                                                    // MSB as
  ...
  FileWriter accfwriter;
  int count1 = 1;
 
  protected void accwriteintoafile(String msg){
	  Calendar c1 = Calendar.getInstance();
	  SimpleDateFormat df = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
	  String timee = df.format(c1.getTime());
	  String msgc = String.format("%04d", count1);
	  try{
	  accfwriter = new FileWriter("/storage/emulated/0/Android/AccelerometerReadingRajesh1210.txt", true);
	  //fwriter.append(serial + " ");
	  accfwriter.append(msgc + " ");
	  accfwriter.append(timee + " ");
	  accfwriter.append(msg + "\n");
	  accfwriter.close();
	  count1 = count1 + 1;
